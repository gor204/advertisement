...
class Advertisement(models.Model):
    ...
    def created_date(self):
        ...
        if self.created_at.date() == timezone.now().date(): # Проверяем, что дата создания объявления == сегодняшняя дата.
    ...
    def __str__(self):
        return f"Advertisements(id={self.id}, title={self.title}, is_auction={self.is_auction})"
